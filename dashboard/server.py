...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        path = urlparse(self.path).path; events = startup_events()
        ...
        if path == "/api/startup-details":
            details = [{"timestamp":e.get("timestamp",""),"result":e.get("result","unknown"),"status":"ready" if e.get("result")=="ready" else "needs_attention"} for e in events]
            self._send(json.dumps(details,ensure_ascii=False).encode()); return
