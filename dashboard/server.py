...
from core.startup_alert_history import StartupAlertHistory
...
DATA = ROOT / "data"
ALERT_HISTORY = StartupAlertHistory()
...
def alert_state(events):
    ...
    history = ALERT_HISTORY.record(alerts)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/startup-alert-history":
            ...
            payload = ALERT_HISTORY.active() if mode == "active" else history if mode == "all" else [x for x in history if x["status"] == "resolved"]
