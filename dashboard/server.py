...
def diagnostic_details(events, status="all"):
    ...
    for event in events:
        ...
        details.append({
            ...
            "issues": [{"message": str(issue), "severity": StartupIssueSeverity.label(issue)} for issue in issues],
        })
    return details
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/startup-details":
            ...
            if status not in {"all", "ready", "needs_attention"}: status = "all"
