...
def history_payload(events):
    return [
        {"timestamp": event.get("timestamp", ""), "result": event.get("result", "unknown"), "score": 100 if event.get("result") == "ready" else 0}
        for event in events
    ]
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/status":
            payload = {
                "name": "Nova AJ", "status": "online",
                "memory": len(read_json("memory.json", [])), "tasks": len(read_json("tasks.json", [])),
                "profile": read_json("profile.json", {}), "skill_proposals": len(read_json("skill_proposals.json", [])),
                "startup_reliability": reliability(events),
            }
            self._send(json.dumps(payload).encode()); return
        if path == "/api/memory":
            self._send(json.dumps(read_json("memory.json", []), ensure_ascii=False).encode()); return
        if path == "/api/tasks":
            self._send(json.dumps(read_json("tasks.json", []), ensure_ascii=False).encode()); return
        if path == "/api/profile":
            self._send(json.dumps(read_json("profile.json", {}), ensure_ascii=False).encode()); return
        if path == "/api/skills":
            self._send(json.dumps(read_json("skill_proposals.json", []), ensure_ascii=False).encode()); return
        if path == "/api/startup-reliability":
            self._send(json.dumps(reliability(events)).encode()); return
        if path == "/api/startup-history":
            self._send(json.dumps(history_payload(events), ensure_ascii=False).encode()); return
