    ...
    for event in events:
        ...
        if severity == "all" or enriched:
            out.append({
                ...
                "status": "ready" if event.get("result") == "ready" else "needs_attention",
                "issues": enriched,
            })
    return out
    ...
    def do_GET(self):
        ...
        if path == "/api/status":
            self._send(json.dumps({
                ...
                "startup_health": summarize(diagnostic_details(events)),
            }).encode())
            return
        if path == "/api/startup-reliability":
            self._send(json.dumps(reliability(events)).encode()); return
        if path == "/api/startup-health":
            self._send(json.dumps(summarize(diagnostic_details(events)), ensure_ascii=False).encode()); return
        ...
        if path == "/api/startup-history":
            self._send(json.dumps([{"timestamp": e.get("timestamp", ""), "result": e.get("result", "unknown"), "score": 100 if e.get("result") == "ready" else 0} for e in events]).encode()); return
