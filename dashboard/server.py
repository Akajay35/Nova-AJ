...
from core.alert_severity_config import load as load_severity_cooldowns, save as save_severity_cooldowns
...
SEVERITY_COOLDOWN_FILE = DATA / "alert_severity_config.json"
ALERT_STORE = StartupAlertStore(DATA / "startup_alert_history.json")
SEVERITY_COOLDOWNS = load_severity_cooldowns(SEVERITY_COOLDOWN_FILE)
ALERT_DEDUP = StartupAlertDeduplicator(cooldown_seconds=load_cooldown(COOLDOWN_FILE), cooldowns=SEVERITY_COOLDOWNS)
...
def alert_state(events):
    ...
    alerts = [route_alert(a) for a in ALERT_DEDUP.filter(raw_alerts)]
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/set-severity-cooldowns":
            q = parse_qs(parsed.query)
            try:
                values = {level: max(0, min(86400, int(q.get(level, [ALERT_DEDUP.cooldowns.get(level, 0)])[0]))) for level in ("critical", "high", "medium", "low")}
            except ValueError:
                self._send(json.dumps({"error":"cooldown values must be integers"}).encode(), status=400); return
            ALERT_DEDUP.cooldowns = save_severity_cooldowns(SEVERITY_COOLDOWN_FILE, values)
            self._send(json.dumps({"severity_cooldowns": ALERT_DEDUP.cooldowns}).encode()); return
        if path == "/api/startup-alerts": self._send(json.dumps(alerts, ensure_ascii=False).encode()); return
        if path == "/api/startup-alert-routing": self._send(json.dumps([route_alert(a) for a in build_alerts(health)], ensure_ascii=False).encode()); return
