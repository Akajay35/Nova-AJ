    ...
    MaterialTheme {
        Surface(Modifier.fillMaxSize()) {
            Scaffold(bottomBar = { NavigationBar { tabs.forEachIndexed { index, title -> NavigationBarItem(selected == index, { selected = index }, icon = { Text(title.take(1)) }, label = { Text(title) }) } } }) { padding ->
                Box(Modifier.padding(padding)) { when (selected) { 0 -> HomeScreen { selected = 1 }; 1 -> ChatScreen(messages, client); 2 -> TrainerScreen(); 3 -> SkillsScreen(); else -> SettingsScreen() } }
            }
        }
    }
}
...
private fun HomeScreen(onChat: () -> Unit) {
    ...
    val speechLauncher = rememberLauncherForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
        ...
        status = if (text.isNullOrBlank()) "No speech detected." else "Heard: $text"
    }
    val permissionLauncher = rememberLauncherForActivityResult(ActivityResultContracts.RequestPermission()) { granted ->
        if (granted) speechLauncher.launch(Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH).apply { putExtra(RecognizerIntent.EXTRA_LANGUAGE_MODEL, RecognizerIntent.LANGUAGE_MODEL_FREE_FORM); putExtra(RecognizerIntent.EXTRA_LANGUAGE, Locale.getDefault()); putExtra(RecognizerIntent.EXTRA_PROMPT, "Talk to Nova-AJ") })
    ...
    Column(Modifier.fillMaxSize().padding(24.dp), verticalArrangement = Arrangement.Center) {
        ...
        Button(onClick = onChat, Modifier.fillMaxWidth()) { Text("Chat with Nova") }
        ...
        Button(onClick = {
            if (androidx.core.content.ContextCompat.checkSelfPermission(context, Manifest.permission.RECORD_AUDIO) == PackageManager.PERMISSION_GRANTED) {
                speechLauncher.launch(Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH).apply { putExtra(RecognizerIntent.EXTRA_LANGUAGE_MODEL, RecognizerIntent.LANGUAGE_MODEL_FREE_FORM); putExtra(RecognizerIntent.EXTRA_LANGUAGE, Locale.getDefault()); putExtra(RecognizerIntent.EXTRA_PROMPT, "Talk to Nova-AJ") })
            } else permissionLauncher.launch(Manifest.permission.RECORD_AUDIO)
        }, Modifier.fillMaxWidth()) { Text("🎙  Talk to Nova") }
...
private fun ChatScreen(messages: MutableList<ChatMessage>, client: NovaApiClient) {
    ...
    Column(Modifier.fillMaxSize().padding(16.dp)) {
        ...
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            Button(onClick = { val text = input.trim(); if (text.isEmpty() || busy) return@Button; messages.add(ChatMessage(text, true)); input = ""; error = null; busy = true; Thread { val result = client.chat(text); runOnUiThread { result.onSuccess { messages.add(ChatMessage(it, false)) }.onFailure { error = "Connection failed: ${it.message ?: "unknown error"}" }; busy = false } }.start() }, enabled = input.isNotBlank() && !busy) { Text(if (busy) "…" else "Send") }
        }
    }
}
