...
class SkillLearningEngine:
    """Turns missing capabilities into reviewable proposals; never installs or executes code automatically."""
    RISKY_PERMISSIONS = {"task_write", "memory_write", "skill_execute", "notification"}
    ...
    def propose(self, name: str, description: str, reason: str, permissions: list[str] | None = None) -> SkillProposal:
        ...
        requested=list(dict.fromkeys(permissions or []))
        risk="high" if any(p in self.RISKY_PERMISSIONS for p in requested) else "low"
        proposal=SkillProposal(name.strip(), description.strip(), reason.strip(), requested, risk)
        items.append(asdict(proposal)); self._write(items); return proposal
