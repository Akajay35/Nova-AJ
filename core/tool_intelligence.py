...
class ToolIntelligence:
    ...
    def _special_match(self, text: str) -> ToolMatch | None:
        patterns = (
            (r"^(?:remember that|remember|save this:)\s+(.+)$", "remember"),
            (r"^(?:search my memory for|search memory for|find in memory)\s+(.+)$", "search_memory"),
            (r"^(?:forget memory|delete memory|forget)\s+(.+)$", "forget"),
        )
